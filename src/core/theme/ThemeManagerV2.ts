import type { RGBAColor, ThemeColors, ThemeConfigType } from '@/types';
...
export class ThemeManagerV2 extends EventEmitter<ThemeEvents> {
  ...
  async setThemeById(presetId: string): Promise<boolean> {
    const preset =
      this.presets.get(presetId) || this.customThemes.get(presetId);
  ...
  static hexToRgba(hex: string, alpha = 1): RGBAColor {
