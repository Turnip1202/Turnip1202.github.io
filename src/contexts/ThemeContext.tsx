...
const applyCssVariables = (isDark: boolean, lightTheme?: ThemeConfigType | null) => {
...
export const ThemeProvider: React.FC<ThemeProviderProps> = ({ children }) => {
  ...
  const [savedLightTheme, setSavedLightTheme] = useState<ThemeConfigType | null>(() => {
    try {
  ...
  useEffect(() => {
    if (savedLightTheme) {
      try {
        localStorage.setItem(SAVED_LIGHT_THEME_KEY, JSON.stringify(savedLightTheme));
      } catch (error) {
  ...
  const setAppTheme = useCallback((newTheme: ThemeConfigType) => {
    if (!isDark) {
